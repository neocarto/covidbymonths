...
dd <- aggregate(data$incid_rad, by=list(Category=data$month), FUN=sum)
colnames(dd) <- c("date","rad")
...
data <- aggregate(data$incid_rad, by=list(id1 = data$dep, id2=data$month), FUN=sum)
colnames(data) <- c("dep","date","rad")
data <- merge(x = data, y = pop, by.x = "dep", by.y = "id")
data <- data[,c("date","dep","rad","pop2020")]
data$radperinh <- data$rad/data$pop2020*100000
csv <- data

dates <- as.character(unique(data$date))  
dates <- dates[order(dates, na.last = NA)]

mypal <- carto.pal(pal1 = "blue.pal", n1 = 10)
...
date = dates[i]
file <- paste0("maps/absolute/rad/rad_",date,".png")
...
total <- sum(data$rad, na.rm = TRUE)

# top15 <- data[order(data$dc, decreasing = TRUE),]
# top15 <- top15[c(1:15),]
# top15 <- merge(dept, top15, by.x = "id", by.y = "dep")

top50 <- data[data$rad >= 200,]
top50 <- merge(dept, top50, by.x = "id", by.y = "dep")

data[,"rad"] <- round(data[,"rad"]/fact,0)


data_unique <- data[data$rad == 1,]
data_multi <-  data[data$rad  > 1,]


if (nrow(data_multi) > 0){
  for (i in 1:dim(data_multi)[1]){
    nb <- as.numeric(data_multi[i,"rad"])[1]
    tmp <- data_multi[i,]
    tmp[,"rad"] <- 1
...
if (nrow(x) != 0){
  d <- cartogram_dorling(st_jitter(x), "rad", l = 45000000, itermax = 1000)
}
...
text(x = 70000, y = 7120000, "PERSONNES GUÉRIES DE LA COVID-19 (RADIATIONS)", cex = 1.5, pos = 4, font = 2, col = col) 
...
breaks <- getBreaks(v = csv$radperinh, nclass = n, method = "fisher")
...
for (i in 1:nb) {
  ...
  date = dates[i]
  file <- paste0("maps/relative/rad/rad_",date,".png")
  
  if (!file.exists(file)){
    
    x <- csv[csv$date == date,]
    x <- merge(dept,x, by.x = "id", by.y="dep")
    total <- sum(x$rad, na.rm = TRUE)
    x$rad <- x$rad * 100000
    
    top50 <- x[x$radperinh >= 20,]
    # top50 <- merge(dept, top50, by.x = "id", by.y = "dep")
    ...
    text(x = 1285000, y = y + 18000 , "NOMBRE DE PERSONNES RADIÉES POUR 100 000 HABITANTS", cex = 0.7, pos = 2, font = 2, col = col)
