...
dd <- aggregate(data$incid_hosp, by=list(Category=data$month), FUN=sum)
colnames(dd) <- c("date","hosp")
...
mypal <- carto.pal(pal1 = "green.pal", n1 = 10)
...
date = dates[i]
file <- paste0("maps/absolute/hosp/hosp_",date,".png")
...
total <- sum(data$hosp, na.rm = TRUE)
...
data[,"hosp"] <- round(data[,"hosp"]/fact,0)


data_unique <- data[data$hosp == 1,]
data_multi <-  data[data$hosp  > 1,]
...
if (nrow(x) != 0){
  d <- cartogram_dorling(st_jitter(x), "hosp", l = 45000000, itermax = 1000)
}
...
text(x = 70000, y = 7120000, "PERSONNES HÔSPITALISÉES EN RAISON DE LA COVID-19", cex = 1.5, pos = 4, font = 2, col = col) 
...
breaks <- getBreaks(v = csv$hospperinh, nclass = n, method = "fisher")
...
for (i in 1:nb) {
  ...
  date = dates[i]
  file <- paste0("maps/relative/hosp/hosp_",date,".png")
  
  if (!file.exists(file)){
    
    x <- csv[csv$date == date,]
    x <- merge(dept,x, by.x = "id", by.y="dep")
    total <- sum(x$hosp, na.rm = TRUE)
    x$hosp <- x$hosp * 100000
    
    top50 <- x[x$hospperinh >= 50,]
    # top50 <- merge(dept, top50, by.x = "id", by.y = "dep")
    ...
    text(x = 70000, y = 6230000, "NOMBRE D'HOPSITALISATIONS", cex = 0.7, pos = 4, font = 2, col = col)
    ...
    text(x = 1285000, y = y + 18000 , "NOMBRE DE PERSONNES HOSPITALISÉES POUR 100 000 HABITANTS", cex = 0.7, pos = 2, font = 2, col = col)
