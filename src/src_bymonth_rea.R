...
dd <- aggregate(data$incid_rea, by=list(Category=data$month), FUN=sum)
colnames(dd) <- c("date","rea")
...
data <- aggregate(data$incid_rea, by=list(id1 = data$dep, id2=data$month), FUN=sum)
colnames(data) <- c("dep","date","rea")
data <- merge(x = data, y = pop, by.x = "dep", by.y = "id")
data <- data[,c("date","dep","rea","pop2020")]
data$reaperinh <- data$rea/data$pop2020*100000
csv <- data

dates <- as.character(unique(data$date))  
dates <- dates[order(dates, na.last = NA)]

mypal <- carto.pal(pal1 = "pink.pal", n1 = 10)
...
date = dates[i]
file <- paste0("maps/absolute/rea/rea_",date,".png")
...
total <- sum(data$rea, na.rm = TRUE)

# top15 <- data[order(data$dc, decreasing = TRUE),]
# top15 <- top15[c(1:15),]
# top15 <- merge(dept, top15, by.x = "id", by.y = "dep")

top50 <- data[data$rea >= 50,]
top50 <- merge(dept, top50, by.x = "id", by.y = "dep")

data[,"rea"] <- round(data[,"rea"]/fact,0)


data_unique <- data[data$rea == 1,]
data_multi <-  data[data$rea  > 1,]


if (nrow(data_multi) > 0){
  for (i in 1:dim(data_multi)[1]){
    nb <- as.numeric(data_multi[i,"rea"])[1]
    tmp <- data_multi[i,]
    tmp[,"rea"] <- 1
...
if (nrow(x) != 0){
  d <- cartogram_dorling(st_jitter(x), "rea", l = 45000000, itermax = 1000)
}
...
text(x = 70000, y = 7120000, "PATIENTS COVID-19 PASSÉS EN RÉANIMATION", cex = 1.5, pos = 4, font = 2, col = col) 
...
text(x = 70000, y = 6230000, "NOMBRE DE PERSONNES", cex = 0.7, pos = 4, font = 2, col = col) 
if (dim(top50)[1] > 0){
...
breaks <- getBreaks(v = csv$reaperinh, nclass = n, method = "fisher")
...
for (i in 1:nb) {
  ...
  date = dates[i]
  file <- paste0("maps/relative/rea/rea_",date,".png")
  
  if (!file.exists(file)){
    
    x <- csv[csv$date == date,]
    x <- merge(dept,x, by.x = "id", by.y="dep")
    total <- sum(x$rea, na.rm = TRUE)
    x$rea <- x$rea * 100000
    
    top50 <- x[x$reaperinh >= 10,]
    # top50 <- merge(dept, top50, by.x = "id", by.y = "dep")
    ...
    text(x = 70000, y = 6230000, "NOMBRE DE PATIENTS", cex = 0.7, pos = 4, font = 2, col = col)
    ...
    text(x = 1285000, y = y + 18000 , "NOMBRE DE PERSONNES EN RÉANIMATION POUR 100 000 HABITANTS", cex = 0.7, pos = 2, font = 2, col = col)
